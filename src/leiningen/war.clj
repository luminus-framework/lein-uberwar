...
(defn default-war-name [project]
  (or (get-in project [:uberwar :war-name])
...
(defn skip-file? [project war-path file]
  (or (re-find #"^\.?#" (.getName file))
      ...
      (some #(re-find % war-path)
            (get-in project [:uberwar :war-exclusions] [#"(^|/)\."]))))

(defn- to-byte-stream [^String s]
  (ByteArrayInputStream. (.getBytes s)))

(defn default-servlet-class [project]
  (let [handler-sym (get-in project [:uberwar :handler])
...
(defn servlet-class [project]
  (or (get-in project [:uberwar :servlet-class])
...
(defn servlet-name [project]
  (or (get-in project [:uberwar :servlet-name])
      (str (get-in project [:uberwar :handler])
           " servlet")))

(defn default-listener-class [project]
  (let [listener-sym (or (get-in project [:uberwar :init])
                         (get-in project [:uberwar :destroy])
                         (get-in project [:uberwar :handler]))
...
(defn listener-class [project]
  (or (get-in project [:uberwar :listener-class])
...
(defn url-pattern [project]
  (or (get-in project [:uberwar :url-pattern])
      "/*"))
    ...
    (if (contains? ring-options :web-xml)
      ...
      (indent-str
        (sexp-as-element
          [:web-app
           (get web-app-attrs
                (get-in project [:uberwar :servlet-version] default-servlet-version)
                {})
           [:listener
...
(defn compile-listener [project]
  (let [init-sym    (get-in project [:uberwar :init])
